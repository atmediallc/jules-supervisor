...
| 20  | Request corrections                  | IMPLEMENTED_(post-audit P0)              | ~~No correction loop mechanism~~ → `REQUEST_CHANGES` auto-executes as a targeted correction via `sendMessage` (pipeline.ts); dispatched corrections persist against the durable session budget; `jules_corrections_submitted_total` metric.                                                                                                                        | Low    | —                                  |
| 21  | Verify corrections                   | IMPLEMENTED_(post-audit P0)              | ~~No re-audit after correction~~ → `classifyDefectResolution` (correction-loop.ts) yields PASS / PARTIAL / FAIL re-audit verdict from defect-addressed + artifact-changed signals.                                                                                                                        | Low    | —                                  |
| 22  | Terminate correction loops safely    | IMPLEMENTED_(post-audit P0)              | ~~MISSING~~ → correction budget ceiling enforced pre-AI (degrades to human review at `BUDGET_MAX_CORRECTIONS_PER_SESSION`), defect fingerprinting (`fingerprintDefect`) prevents identical corrections, and the loop detector escalates repeated identical prompts to a human.                                                                                                                        | Low    | —                                  |
